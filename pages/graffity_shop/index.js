...
class App {
  ...
  set_structure() {
    this.graffity_types_container = document.querySelector(
      "#graffity_types_container"
    );
  ...
  fetch_data() {
    ...
    for (let id in Graffity_table) {
      ...
      img_elem.src =
        this.assetPath + "graffities/vector/" + Graffity_table[id].v1;
  ...
  add_listPageEvent() {
    ...
    for (let value of this.listElements) {
      value.addEventListener("mouseover", () => {
        ...
        value.firstElementChild.style.filter =
          "invert(97%) sepia(100%) saturate(1557%) hue-rotate(103deg) brightness(105%) contrast(102%)";
      });
      value.addEventListener("mouseout", () => {
        value.style.background = "rgba(20, 20, 20, .0) ";
        value.firstElementChild.style.filter =
          "invert(91%) sepia(93%) saturate(29%) hue-rotate(197deg) brightness(107%) contrast(100%)";
      });
      value.addEventListener("click", (event) => {
        this.show_types(event);
      });
    }
  }
  ...
  show_types(e) {
    ...
    this.v0.src = this.assetPath + "graffities/vector/" + Graffity_table[id].v1;
