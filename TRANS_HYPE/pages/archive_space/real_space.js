...
export class real_space {
  ...
  _setupModel() {
    ...
    // const objLoader = new OBJLoader();
    objLoader.load(this.assetPath + "maps/archive/spot.obj", (obj) => {
      this._scene.add(obj);
  ...
  move_camera(event) {
    ...
    const min = 3;
    const max = 15;
    this._camera.position.z = Math.min(
      Math.max(this._camera.position.z + event.deltaY * 0.002, min),
      max
    );
  }
      ...
      if (intersects[0]) {
        for (let i = 0; i < this.raycasting_obj.length; i++) {
          if (intersects[0].object == this.raycasting_obj[i]) {
            ...
            this.p01.src = this.assetPath + "archive_pictures/" + info.pic1;
            this.p02.src = this.assetPath + "archive_pictures/" + info.pic2;
            this.p03.src = this.assetPath + "archive_pictures/" + info.pic3;
            this.hoverable = 0;
      ...
      if (compare_plusXY + compare_minusXY == 1) {
        if (Math.abs(this.mouse.x) > 0.3) {
          this._camera.position.x = Math.min(
            Math.max(this._camera.position.x + this.mouse.x * fraction, minX),
            maxX
          );
        }
      } else {
        if (Math.abs(this.mouse.y) > 0.3) {
          this._camera.position.z = Math.min(
            Math.max(this._camera.position.z - this.mouse.y * fraction, minZ),
            maxZ
          );
        }
      }
