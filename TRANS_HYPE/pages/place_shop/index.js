...
class App {
  constructor() {
    ...
    for (let area_id in Area_table) {
      ...
      img_elem.src = this.assetPath + "maps/virtual/2d/" + area_id + ".svg";
    ...
    const bgtexture = new THREE.TextureLoader().load(
      this.assetPath + "textures/background/grid_b.png"
    );
  ...
  add_listPageEvent() {
    for (let value of this.listElements) {
      value.addEventListener("mouseover", () => {
        ...
        document.querySelector("#" + value.id + "_2d_map").style.filter =
          "invert(99%) sepia(99%) saturate(7499%) hue-rotate(102deg) brightness(104%) contrast(102%)";
      });
      value.addEventListener("mouseout", () => {
        ...
        document.querySelector("#" + value.id + "_2d_map").style.filter =
          "invert(100%) sepia(100%) saturate(0%) hue-rotate(292deg) brightness(108%) contrast(108%)";
      });
  ...
  reset_model(name) {
    ...
    this.objects = [];
    let url = this.assetPath + "maps/virtual/3d/areas/" + Area_table[name].area;
